      ...
      <main className="relative z-10 max-w-4xl mx-auto px-4 pb-8">
        ...
        >
          ...
          {getContextualMessage() && (
            <motion.div
              className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-2xl p-4 mb-6 border border-white/20 dark:border-gray-700/20"
        ...
        {/* Journal Entry Form */}
        <motion.div
          className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20 dark:border-gray-700/20"
        ...
        >
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-display font-semibold text-zen-sage-800 dark:text-gray-200 flex items-center">
                <Lightbulb className="w-5 h-5 mr-2 text-zen-mint-500" />
                Today's Reflection
              </h3>
              <button
                onClick={handleGenerateNewPrompt}
                disabled={isLoadingPrompt}
                className="flex items-center space-x-1 px-3 py-1 text-sm text-zen-sage-600 dark:text-gray-400 hover:text-zen-sage-800 dark:hover:text-gray-200 hover:bg-white/50 dark:hover:bg-gray-700/50 rounded-full transition-all duration-300 disabled:opacity-50"
              >
                <RefreshCw className={`w-4 h-4 ${isLoadingPrompt ? 'animate-spin' : ''}`} />
                <span>New Prompt</span>
              </button>
            </div>
            <div className="bg-zen-mint-50 dark:bg-gray-700/50 rounded-2xl p-4 border border-zen-mint-200 dark:border-gray-600">
              <p className="text-zen-sage-700 dark:text-gray-300 font-medium italic">
                "{getDailyPrompt()}"
              </p>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/50 dark:bg-gray-700/50 border border-zen-sage-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-zen-mint-400 focus:border-transparent transition-all duration-300 text-zen-sage-800 dark:text-gray-200 placeholder-zen-sage-400 dark:placeholder-gray-500"
            />
          </div>
          ...
          {/* Mood Confirmation */}
          <AnimatePresence>
            {showMoodConfirmation && selectedMood && (
              <motion.div
                className="mb-4 bg-zen-mint-50 dark:bg-gray-700/50 rounded-2xl p-4 border border-zen-mint-200 dark:border-gray-600"
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
              >
                <div className="flex items-center space-x-2">
                  <CheckCircle className="w-5 h-5 text-zen-mint-500" />
                  <p className="text-zen-sage-700 dark:text-gray-300 font-medium">
                    Got it! You're feeling {moods.find(m => m.level === selectedMood)?.label.toLowerCase()}.
                  </p>
                </div>
              </motion.div>
            )}
          ...
          <AnimatePresence>
            {showMoodSuggestion && aiDetectedMood && (
              <motion.div
                className="mb-4 bg-zen-lavender-50 dark:bg-gray-700/50 rounded-2xl p-4 border border-zen-lavender-200 dark:border-gray-600"
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
              >
                <div className="flex items-start space-x-3">
                  <Sparkles className="w-5 h-5 text-zen-lavender-500 mt-0.5" />
                  <div className="flex-1">
                    <p className="text-zen-sage-700 dark:text-gray-300 font-medium mb-2">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="px-3 py-1 bg-zen-lavender-500 text-white rounded-lg text-sm font-medium hover:bg-zen-lavender-600 transition-colors"
                      >
                        Yes, update it
                      </button>
                      <button
                        onClick={handleDismissMoodSuggestion}
                        className="px-3 py-1 bg-zen-sage-200 dark:bg-gray-600 text-zen-sage-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-zen-sage-300 dark:hover:bg-gray-500 transition-colors"
                      >
                        No, keep current
                      </button>
                    </div>
          ...
          <AnimatePresence>
            {showMoodQuote && moodQuote && (
              <motion.div
                className="mb-4 bg-zen-peach-50 dark:bg-gray-700/50 rounded-2xl p-4 border border-zen-peach-200 dark:border-gray-600"
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
              >
                <div className="flex items-start space-x-3">
                  <Heart className="w-5 h-5 text-zen-peach-500 mt-0.5" />
                  <div>
                    <p className="text-zen-sage-700 dark:text-gray-300 font-medium italic mb-1">
                      "{moodQuote.quote}"
                    </p>
                    {moodQuote.attribution && (
                      <p className="text-zen-sage-500 dark:text-gray-400 text-sm">
                        — {moodQuote.attribution}
                      </p>
                    )}
                  </div>
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <textarea
                ...
                rows={8}
                className="w-full px-4 py-4 bg-white/50 dark:bg-gray-700/50 border border-zen-sage-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-zen-mint-400 focus:border-transparent transition-all duration-300 text-zen-sage-800 dark:text-gray-200 placeholder-zen-sage-400 dark:placeholder-gray-500 resize-none"
              />
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-zen-mint-400 to-zen-mint-500 text-white font-medium rounded-xl hover:from-zen-mint-500 hover:to-zen-mint-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-xl"
            >
              {isSubmitting ? (
                <>
        ...
        <AnimatePresence>
          {showAffirmation && affirmation && (
            <motion.div
              className="mt-6 bg-gradient-to-r from-zen-lavender-50 to-zen-peach-50 dark:from-gray-800/50 dark:to-gray-700/50 rounded-3xl p-6 shadow-lg border border-zen-lavender-200 dark:border-gray-600"
            ...
            >
              <div className="flex items-start space-x-4">
                <div className="flex-shrink-0">
                  <div className="w-12 h-12 bg-gradient-to-r from-zen-lavender-400 to-zen-peach-400 rounded-full flex items-center justify-center">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-zen-sage-700 dark:text-gray-300 leading-relaxed font-medium">
                    {affirmation}
                  </p>
                  {affirmationError && (
        ...
        {alreadyJournaledToday && !showSuccess && (
          <motion.div
            className="mt-6 bg-zen-mint-50 dark:bg-gray-800/50 rounded-3xl p-6 border border-zen-mint-200 dark:border-gray-600"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
          >
            <div className="flex items-center space-x-3">
              <CheckCircle className="w-6 h-6 text-zen-mint-500" />
              <div>
                ...
                <p className="text-zen-sage-600 dark:text-gray-400">
                  Great job maintaining your streak. Feel free to add another entry if you'd like.
                </p>
              </div>
            </div>
          </motion.div>
